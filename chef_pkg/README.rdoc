...
This cookbook is only intended for Chef installations from distribution
packages. Currently this means Debian, Ubuntu, CentOS or RHEL.
...
The default server recipe (chef_pkg::server) sets up two Merb Mongrel workers for the webui/api (port 4000) and openid (port 4001).
